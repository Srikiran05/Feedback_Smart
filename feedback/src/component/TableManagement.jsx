...
const TableManagement = () => {
    ...
    const fetchTables = async () => {
        try {
            setLoading(true);
            setError('');
            const response = await axios.get('http://192.168.0.5:5000/api/tables');
    ...
    const handleAddTable = async (e) => {
        ...
        try {
            await axios.post('http://192.168.0.5:5000/api/tables', newTableData);
            fetchTables();
    ...
    const handleDeleteTable = async (tableId) => {
        if (window.confirm('Are you sure you want to delete this table?')) {
            try {
                await axios.delete(`http://192.168.0.5:5000/api/tables/${tableId}`);
                fetchTables();
            } catch (err) {
